...
const chapterCard = require('../skill/long-novel-writer/scripts/chapter-card');
...
function fixtureProject() {
  ...
  fs.appendFileSync(path.join(project, 'outline', 'chapter-beats.md'), '| 1 | Lin | make the collector answer | the exit is blocked | the receipt exposes a witness | loses the safe route | a witness waits inside | fear becomes resolve | the lock clicks twice |\n', 'utf8');
  fs.writeFileSync(path.join(project, 'manuscript', 'ch-0001-雨夜.md'), '# 雨夜\n\n林越把欠条按在柜台上，雨水顺着他的手背滴落。\n\n巷口的人没有散，反而把退路堵住了。\n', 'utf8');
  return project;
}

function completeEditorialChecks() {
  const evidence = report().scene_evidence.goal.evidence;
  return readerReview.EDITORIAL_DIMENSIONS.map((dimension) => ({ id: dimension.id, verdict: 'pass', evidence, note: `Literal scene evidence supports ${dimension.id}.` }));
}

function completeObligationChecks(project) {
  const evidence = report().scene_evidence.goal.evidence;
  const written = chapterCard.write(project, { chapter: '1' });
  assert.equal(written.card.status, 'ready', JSON.stringify(written.errors));
  return written.card.chapter_obligations.map((obligation) => ({ id: obligation.id, verdict: 'pass', evidence, note: `The literal scene evidence delivers ${obligation.id}.` }));
}
...
test('schema 1.4 binds a focused editorial-dimension pass to literal chapter evidence', () => {
  ...
  const result = readerReview.validate(project, { chapter: '1', file: relative });
  assert.equal(result.data.schema_version, '1.6');
...
test('schema 1.6 binds every exact chapter obligation to cold-reader evidence', () => {
  const project = fixtureProject();
  const relative = 'analysis/chapter-reader-review-ch0001-r01.json';
  const obligations = completeObligationChecks(project);
  const full = report({ schema_version: '1.6', editorial_dimension_checks: completeEditorialChecks(), hook_agenda_checks: [], chapter_obligation_checks: obligations });
  fs.writeFileSync(path.join(project, relative), JSON.stringify(full, null, 2), 'utf8');
  const passed = readerReview.validate(project, { chapter: '1', file: relative });
  assert.equal(passed.data.schema_version, '1.6');
  assert.deepEqual(passed.data.chapter_obligation_failures, []);
  assert.equal(passed.data.should_revise, false);

  fs.writeFileSync(path.join(project, relative), JSON.stringify({ ...full, chapter_obligation_checks: obligations.slice(1) }, null, 2), 'utf8');
  assert.throws(() => readerReview.validate(project, { chapter: '1', file: relative }), (error) => error.code === 'CHAPTER_READER_REVIEW_INVALID');

  const failed = obligations.map((item) => item.id === 'beat_turn' ? { ...item, verdict: 'fail', note: 'The prose has a generic choice but does not deliver the assigned receipt reveal.' } : item);
  fs.writeFileSync(path.join(project, relative), JSON.stringify({ ...full, verdict: 'revise', chapter_obligation_checks: failed }, null, 2), 'utf8');
  const revised = readerReview.validate(project, { chapter: '1', file: relative });
  assert.deepEqual(revised.data.chapter_obligation_failures, ['beat_turn']);
  assert.equal(revised.data.should_revise, true);
});
