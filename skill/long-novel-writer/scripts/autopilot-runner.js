...
function chapterPrompt(project, chapter, transactionResult) {
  return [
    ...
    'Read the local skill, author intent, current focus, reader contract, platform contract, chapter card, chapter beat, context pack, current state, unresolved hooks, feedback rules, style contract, character contracts, foreshadowing progress, hook agenda, resource window, pacing ledger, quality guidance, and repair-debt guidance when present. Every active feedback rule, adopted style signal, and active character contract is a reader-experience constraint, not a note to summarize. Let each on-page contract character act from an own goal, pressure, information boundary, and voice/action profile. Never copy source-specific names, plots, or wording from the evidence behind a style signal.',
